...
import { MODE_COLOURS, MODE_LABEL, type Behaviour } from './behaviour'
  ...
  return (
    <div className="mapwrap hero">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="map" role="img" aria-label={`${c.corridor.street} from ${c.corridor.to} to ${c.corridor.from}`}>
        ...
        {c.boundaries.map(b => <line key={b.id} x1={x(b.pos_m)} x2={x(b.pos_m)} y1={16} y2={H - 30} className="crossstreet" />)}
        ...
        {c.segments.map(s => {
          ...
          return (
            <g key={s.id} className={`seg ${on ? 'on' : ''} ${dim ? 'dim' : ''}`} onClick={() => onSelect(on ? null : s.id)} tabIndex={0} role="button" aria-pressed={on} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onSelect(on ? null : s.id) } }}>
              ...
              {(() => { const ms = b?.q4_mode.segment[s.id]; if (!ms) return null; const bw = Math.max(4, x1 - x0 - 6); let acc = 0; return <g className="modebars"><title>{s.name}: {(['car', 'foot', 'bike', 'subway'] as const).map(k => `${ms[k]}% ${MODE_LABEL[k]}`).join(', ')}</title>{(['car', 'foot', 'bike', 'subway'] as const).map(k => { const wdt = (ms[k] / 100) * bw; const el = <rect key={k} x={x0 + 3 + acc} y={roadY + 78} width={wdt} height={10} fill={MODE_COLOURS[k]} />; acc += wdt; return el })}</g> })()}
            </g>
          )
        })}
        ...
        <g className="maplegend" transform={`translate(${padL - 24}, ${H - 10})`}>
          <text x={0} y={0}>who passes through, latest count:</text>
          {(['car', 'foot', 'bike', 'subway'] as const).map((k, i) => <g key={k} transform={`translate(${200 + i * 110}, 0)`}><rect x={0} y={-9} width={14} height={9} rx={2} fill={MODE_COLOURS[k]} /><text x={18} y={0}>{MODE_LABEL[k]}</text></g>)}
          <rect x={660} y={-9} width={14} height={9} rx={2} fill="url(#works)" /><text x={678} y={0}>street works this month</text>
        </g>
      </svg>
